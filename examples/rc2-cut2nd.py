...
class RC2(object):
    ...
    def __init__(self, formula, solver='g3', adapt=False, conf_budget=1000, differential=1, exhaust=False,
        ...
        self.sum_indices = {}  # a mapping from sum assumptions to their
                               # indices, where an index is a pair
                               # (prefix, bound) denoting the constraint that
                               # the sum over prefix `prefix` of the associated
                               # core is at most `bound`.
        self.prefix_lists = {} # a mapping from seqcounter objects to prefix lists,
                               # lists of pairs (prefix_len, marginal_weight).
    ...
    def reveal_sum_assump(self, sobj, prefix, bound):
        """
            Given a sequential counters object `sobj`, introduce the sum
            assumption representing that the sum of the literals in prefix
            `prefix` of the core is at most `bound`. This is
            :math:`\\overline{o}_i^r` where :math:`i` is `bound` and :math:`r`
            is `prefix`.
        """
        assert bound >= 0
    ...
    def get_sum_assumption(self, sobj, prefix_len, bound):
        assert 0 <= bound
        assert bound < prefix_len
        assert prefix_len <= len(sobj.lits)

        # Increase the bound so we are guaranteed to have the literal for the
        # desired sum assumption.
        sobj.increase(ubound=bound, top_id=self.pool.top)
        self.pool.top = sobj.top_id
        if sobj.nof_new:
            for cl in sobj.cnf.clauses[-sobj.nof_new:]:
                self.oracle.add_clause(cl)

        # Return the literal for the desired sum assumption.
        lit = sobj.get_constraint(prefix_len, bound)
    ...
    def get_coerced_prefixes(self, lits):
        ...
        if len(unique_weights) >= 2:
            snd_weight = sorted(unique_weights)[1]
            index = weights.index(snd_weight)
            blocks = [(sorted_lits[:index], min_weight), (sorted_lits[index:], snd_weight)]
        else:
    ...
    def update_sum(self, assump):
        ...
        raise Exception("update_sum is deprecated for rc2-cuscus")
    ...
    def set_bound(self, tobj, rhs, weight=None):
        ...
        raise Exception("set_bound is deprecated for rc2-cuscus")
    ...
    def filter_assumps(self):
        ...
        for l in list(self.garbage):
            if l in self.sum_indices:
                del self.sum_indices[l]
            if l in self.wght:
                del self.wght[l]
...
class RC2Stratified(RC2, object):
    ...
    def compute(self):
        ...
        if self.done == 0 and self.levl != None:
            ...
            if self.levl is None or self.done >= len(self.blop):
                # There are no soft clauses. We just need to solve the hard clauses.
                assert not self.sels
                self.oracle.solve()
        ...
        if self.model is None:
            # The hard clauses are unsatisfiable, which we signal by returning None.
            return
...
if __name__ == '__main__':
    ...
    if files:
        ...
        formula = WCNFPlus()
        for hard_clause in wcnf.hard_clauses:
            formula.append(hard_clause)
        for soft_clause, weight in wcnf.soft_clauses:
            formula.append(soft_clause, weight=weight)
